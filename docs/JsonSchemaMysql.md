...
**sort_buffer_size** | Option<**i32**> | Sort buffer size in bytes for ORDER BY optimization. Default is 262144 (256K) | [optional]
**innodb_thread_concurrency** | Option<**i32**> | Defines the maximum number of threads permitted inside of InnoDB. Default is 0 (infinite concurrency - no limit) | [optional]
**innodb_write_io_threads** | Option<**i32**> | The number of I/O threads for write operations in InnoDB. Default is 4. Changing this parameter will lead to a restart of the MySQL service. | [optional]
**innodb_ft_min_token_size** | Option<**i32**> | Minimum length of words that are stored in an InnoDB FULLTEXT index. Changing this parameter will lead to a restart of the MySQL service. | [optional]
**innodb_change_buffer_max_size** | Option<**i32**> | Maximum size for the InnoDB change buffer, as a percentage of the total size of the buffer pool. Default is 25 | [optional]
...
**tmp_table_size** | Option<**i32**> | Limits the size of internal in-memory tables. Also set max_heap_table_size. Default is 16777216 (16M) | [optional]
...
**net_read_timeout** | Option<**i32**> | The number of seconds to wait for more data from a connection before aborting the read. | [optional]
**innodb_lock_wait_timeout** | Option<**i32**> | The length of time in seconds an InnoDB transaction waits for a row lock before giving up. Default is 120. | [optional]
...
**net_buffer_length** | Option<**i32**> | Start sizes of connection buffer and result buffer. Default is 16384 (16K). Changing this parameter will lead to a restart of the MySQL service. | [optional]
...
**innodb_online_alter_log_max_size** | Option<**i32**> | The upper limit in bytes on the size of the temporary log files used during online DDL operations for InnoDB tables. | [optional]
**interactive_timeout** | Option<**i32**> | The number of seconds the server waits for activity on an interactive connection before closing it. | [optional]
**innodb_log_buffer_size** | Option<**i32**> | The size in bytes of the buffer that InnoDB uses to write to the log files on disk. | [optional]
**max_allowed_packet** | Option<**i32**> | Size of the largest message in bytes that can be received by the server. Default is 67108864 (64M) | [optional]
