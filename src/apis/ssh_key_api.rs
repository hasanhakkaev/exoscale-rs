...
/// struct for passing parameters to the method [`register_ssh_key`]
#[derive(Clone, Debug)]
pub struct RegisterSshKeyParams {
    pub register_ssh_key_request: crate::models::RegisterSshKeyRequest,
}
...
) -> Result<crate::models::Operation, Error<DeleteSshKeyError>> {
...
) -> Result<crate::models::SshKey, Error<GetSshKeyError>> {
    let local_var_configuration = configuration;
...
pub async fn list_ssh_keys(
    configuration: &configuration::Configuration,
) -> Result<crate::models::ListSshKeys200Response, Error<ListSshKeysError>> {
    let local_var_configuration = configuration;
    ...
    let local_var_uri_str = format!("{}/ssh-key", local_var_configuration.base_path);
    let mut local_var_req_builder =
        local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
...
pub async fn register_ssh_key(
...
) -> Result<crate::models::Operation, Error<RegisterSshKeyError>> {
    let local_var_configuration = configuration;

    // unbox the parameters
    let register_ssh_key_request = params.register_ssh_key_request;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<RegisterSshKeyError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
